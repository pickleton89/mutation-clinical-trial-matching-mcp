...
from utils.node import Node, AsyncNode, Flow, AsyncFlow
...
class ExampleQueryNode(Node):
    ...
    def post(self, shared: Dict[str, Any], prep_result: str, exec_result: Dict[str, Any]) -> Optional[str]:
...
class ExampleProcessNode(Node):
    ...
    def post(self, shared: Dict[str, Any], prep_result: Dict[str, Any], exec_result: Dict[str, Any]) -> Optional[str]:
...
class ExampleReviewNode(Node):
    ...
    def post(self, shared: Dict[str, Any], prep_result: Dict[str, Any], exec_result: str) -> Optional[str]:
...
def example_simple_chaining():
    ...
    print(f"Initial query: {shared['query']}")
...
def example_branching_pattern():
    ...
    for scenario in scenarios:
        ...
        print(f"Final result: {result.get('final_result', 'no final result')}")
...
def example_complex_workflow():
    ...
    for test_case in test_cases:
        print(f"\\n--- Processing: {test_case['document_path']} ---")
        shared = {"document_path": test_case["document_path"]}
        result = flow.run(shared)
        print(f"Document type: {result.get('document_type', 'unknown')}")
        print(f"Processing result: {result.get('processing_result', 'no result')}")
...
if __name__ == "__main__":
    ...
    """
    print("PocketFlow Pattern Alignment Examples")
    print("=" * 50)
    ...
    print("1. ✅ Node chaining with >> operator")
    print("2. ✅ Branching with - operator")
    print("3. ✅ Automatic node registration")
    print("4. ✅ PocketFlow-compliant syntax")