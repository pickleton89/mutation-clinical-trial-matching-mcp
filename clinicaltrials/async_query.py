...
async def _async_query_clinical_trials_impl(
    ...
    if not mutation or not isinstance(mutation, str) or len(mutation.strip()) == 0:
        ...
        increment(
            "clinicaltrials_api_validation_errors_async", tags={"error_type": "invalid_mutation"}
        )
    ...
    if not isinstance(min_rank, int) or min_rank < 1:
        ...
        increment(
            "clinicaltrials_api_validation_warnings_async",
            tags={"warning_type": "invalid_min_rank"},
        )
        min_rank = 1
    ...
    if not isinstance(max_rank, int) or max_rank < min_rank:
        ...
        increment(
            "clinicaltrials_api_validation_warnings_async",
            tags={"warning_type": "invalid_max_rank"},
        )
        max_rank = min_rank + 9
    ...
    with timer("clinicaltrials_api_request_async", tags={"mutation": mutation}):
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                executor, _sync_query_clinical_trials_impl, mutation, min_rank, max_rank, timeout
            )
            ...
            if "error" not in result:
                ...
                histogram(
                    "clinicaltrials_api_study_count_async", study_count, tags={"mutation": mutation}
                )
            ...
            logger.error(
                f"Unexpected error in async query: {e}",
                extra={"mutation": mutation, "error": str(e), "action": "async_api_request_error"},
            )
...
async def query_multiple_mutations_async(
    ...
    start_time = time.time()
    logger.info(
        f"Starting batch async query for {len(mutations)} mutations",
        extra={
            "mutations": mutations,
    ...
    histogram(
        "clinicaltrials_api_batch_duration",
        batch_duration,
        tags={"batch_size": str(len(mutations))},
    )
    ...
    for result in results:
        if isinstance(result, Exception):
            logger.error(
                f"Batch query exception: {result}",
                extra={
                    "batch_size": len(mutations),
                    "error": str(result),
    ...
    histogram(
        "clinicaltrials_api_batch_success_count",
        successful_queries,
        tags={"batch_size": str(len(mutations))},
    )
    gauge(
        "clinicaltrials_api_last_batch_success_rate",
        successful_queries / len(mutations) if mutations else 0,
    )

    logger.info(
        f"Batch async query completed: {successful_queries}/{len(mutations)} successful in {batch_duration:.2f}s",
        extra={
            "mutations": mutations,
            "batch_size": len(mutations),
