...
class MockNode(Node):
    ...
    def post(self, shared: Dict[str, Any], prep_result: str, exec_result: str) -> Optional[str]:
...
class MockAsyncNode(AsyncNode):
    ...
    async def post(self, shared: Dict[str, Any], prep_result: str, exec_result: str) -> Optional[str]:
        self.post_called = True
        shared[f"{self.name}_result"] = exec_result
        return self.get_next_node_id()
